...
class KimiV4ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: TokenizerLike, tools: list[Tool] | None = None):
        ...
        # literal strings [open]/[close]/[sep] (the open tokenizer keeps them as
        # special=False added tokens, and adjust_request suppresses spacing). As
        # DEFENSE-IN-DEPTH we still tolerate optional whitespace WITHIN a marker
        # (e.g. "[open] tools [sep]") in case some serving path leaves vLLM's
        # added-token spacing on -- this `\s*` is a no-op on clean input, so the
        # normal byte-exact path is unaffected. We do NOT strip body whitespace,
        # so clean content stays byte-exact.
    ...
    def adjust_request(
        ...
        # The XTML markers ([open]/[close]/[sep], [open]response[sep] ...) must
        ...
        # The markers are registered as added tokens (special=False) in the open
        # tokenizer, so the detokenizer treats each as its own sub-text and, when
        # the effective flag is True, joins them with spaces -> "[open] response
        # [sep]", which the regexes below would NOT match. Forcing BOTH flags off
        # is the only way to suppress that spacing (and special=False already
        # keeps the markers from being stripped). We set both unconditionally:
        # the response channel is unwrapped from these markers even with no tools.
    ...
    def _extract_response_content(self, current_text: str) -> str | None:
        ...
        if candidates:
            ...
            overlap = max(
                partial_tag_overlap(current_text, self.response_close),
                partial_tag_overlap(current_text, self.tools_open),
            )
    ...
    def extract_tool_calls(
        ...
        try:
            ...
            logger.exception("Error extracting v4 tool calls.")
            return ExtractedToolCallInformation(
                tools_called=False, tool_calls=[], content=model_output
            )
