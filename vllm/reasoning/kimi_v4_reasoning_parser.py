...
"""Reasoning parser for the Kimi v4 (XTML) chat format.

This strips the ``think`` channel out of generated text and hands the remainder
(``response`` + ``tools`` channels) downstream. Kimi v4 wraps the thinking
...
Two subtleties drive the implementation:
  * Unlike Kimi-K2 (a single ``<think>`` token), each v4 marker is a 3-token
...
``{"enable_thinking": False}``, i.e. instruct mode) the parser delegates
wholesale to ``IdentityReasoningParser`` -- there is simply no think channel to
extract.
"""
...
class KimiV4ReasoningParser(ReasoningParser):
    """Reasoning parser for the Kimi v4 (XTML) think channel."""
    ...
    def __init__(self, tokenizer: PreTrainedTokenizerBase, *args, **kwargs):
        ...
        self._identity_parser: IdentityReasoningParser | None = (
            None if thinking else IdentityReasoningParser(tokenizer, *args, **kwargs)
        )
        ...
        self._think_open_re = re.compile(r"\[open\]\s*think\s*\[sep\]")
        self._think_close_re = re.compile(r"\[close\]\s*think\s*\[sep\]")
    ...
    def adjust_request(
        ...
        # The think markers ([open]think[sep] / [close]think[sep]) must reach the
        # parser as CONTIGUOUS literal text. vLLM's detokenizer uses
        #   effective_spaces_between =
        #       skip_special_tokens OR spaces_between_special_tokens
        # and the markers are added tokens (special=False) -> if that flag is True
        # they get joined with spaces ("[open] think [sep]") and find() fails.
        # Forcing BOTH off is the only way to suppress the spacing. Mirrored in
        # KimiV4ToolParser.adjust_request so reasoning-only deployments are also
        # correct (the tool parser's hook is not guaranteed to run).
    ...
    def is_reasoning_end(self, input_ids: Sequence[int]) -> bool:
        ...
        return _subseq_index(input_ids, self._think_close_ids) != -1
    ...
    def is_reasoning_end_streaming(
        ...
        # close marker spans tokens; check on the full current sequence
        return _subseq_index(input_ids, self._think_close_ids) != -1
    ...
    def extract_reasoning(
        ...
        if m_close is not None:
            ...
            return (reasoning or None, rest or None)
    ...
    def extract_reasoning_streaming(
        ...
        if m_close is not None:
            ...
            return DeltaMessage(
                reasoning=reasoning_delta or None, content=content or None
            )
