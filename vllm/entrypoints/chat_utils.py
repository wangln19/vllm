...
class _XTMLToolResultTarget:
    tool: str
    index: int
    order: int


def _xtml_tool_call_target(
    tool_call: ChatCompletionMessageToolCallParam,
    index: int,
) -> _XTMLToolResultTarget | None:
    ...
    return _XTMLToolResultTarget(tool=name, index=index, order=index - 1)


def _xtml_tool_call_id_aliases(
    tool_call: ChatCompletionMessageToolCallParam,
    target: _XTMLToolResultTarget,
) -> tuple[str, ...]:
    aliases: list[str] = []
    tool_call_id = tool_call.get("id")
    if tool_call_id:
        aliases.append(str(tool_call_id))

    aliases.append(f"{target.tool}:{target.index - 1}")

    return tuple(dict.fromkeys(aliases))


def _xtml_tool_result_sort_key(
    message: ChatCompletionMessageParam,
    targets_by_id: dict[str, _XTMLToolResultTarget],
    targets_by_pair: dict[tuple[str, int], _XTMLToolResultTarget],
) -> _XTMLToolResultTarget | None:
    tool_call_id = message.get("tool_call_id")
    if tool_call_id is not None and str(tool_call_id) in targets_by_id:
        return targets_by_id[str(tool_call_id)]

    tool = message.get("tool") or message.get("name")
    index = message.get("index")
    if tool is None or index is None:
        return None
    ...
    return targets_by_pair.get((str(tool), index_int))


def _normalize_xtml_tool_result_block(
    block: list[ChatCompletionMessageParam],
    targets_by_id: dict[str, _XTMLToolResultTarget],
    targets_by_pair: dict[tuple[str, int], _XTMLToolResultTarget],
) -> list[ChatCompletionMessageParam]:
    keyed: list[tuple[_XTMLToolResultTarget, int, ChatCompletionMessageParam]] = []
    for i, message in enumerate(block):
        target = _xtml_tool_result_sort_key(message, targets_by_id, targets_by_pair)
        if target is None:
            return block

        normalized = dict(message)
        normalized["tool"] = target.tool
        normalized["index"] = target.index
        keyed.append((target, i, cast(ChatCompletionMessageParam, normalized)))

    keyed.sort(key=lambda item: (item[0].order, item[1]))
    return [message for _, _, message in keyed]


def normalize_xtml_tool_result_messages(
    messages: list[ChatCompletionMessageParam],
) -> list[ChatCompletionMessageParam]:
    """Align XTML tool-result messages with the preceding assistant calls."""
    ...
    while i < len(messages):
        ...
        for index, tool_call in enumerate(tool_calls, start=1):
            target = _xtml_tool_call_target(tool_call, index)
            if target is None:
                continue

            targets_by_pair[(target.tool, target.index)] = target
            for alias in _xtml_tool_call_id_aliases(tool_call, target):
        ...
        normalized.extend(
            _normalize_xtml_tool_result_block(
                messages[block_start:i],
                targets_by_id,
                targets_by_pair,
            )
        )
